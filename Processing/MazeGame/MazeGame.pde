...
void setup()
{
   ...
   gui = new GameGUI(ROWS, COLS, NUM_MONSTERS, NUM_TREASURES, "Princess Elsa");
   smooth();
   snowSetup();
...
int numBalls=100;
        float[] snowX = new float[numBalls];
        float[] snowY = new float[numBalls];
        int[] flakeSize = new int[numBalls];
        int[] direction= new int[numBalls];
        ...
        public void drawSnow()
        {
          ...
          {
            ellipse(snowX[i], snowY[i], flakeSize[i], flakeSize[i]);
            
            if(direction[i] == 0)
            {
              snowX[i] += map(flakeSize[i], 1, 5, .1, .5);
            }
            else
            {
              snowX[i] -= map(flakeSize[i], 1, 5, .1, .5);
            ...
            snowY[i] += flakeSize[i] + direction [i];
            
            if(snowX[i] > width + flakeSize[i] || snowX[i] < -flakeSize[i] 
              || snowY[i] > height + flakeSize[i])
            {
        ...
        public void snowSetup()
        {
          for(int i = 0; i < numBalls; i++)
          {
            flakeSize[i] = round(random(1, 5));
            snowX[i] = random(0, width);
